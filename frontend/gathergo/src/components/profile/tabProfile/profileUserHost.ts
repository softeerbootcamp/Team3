...
class profileUserHost {
  ...
  render() {
    ...
    this.element.innerHTML += `
        <td>${new Date(this.userHostData.meetingDay).getFullYear()}년 ${
      new Date(this.userHostData.meetingDay).getMonth() + 1
    }월 ${new Date(this.userHostData.meetingDay).getDate()}일</td>
        `;
